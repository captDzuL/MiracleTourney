...
export function AwardReview({ awards, decisions, onDecisionChange }: AwardReviewProps) {
        ...
        return <article data-award={award.award} key={award.award} className="min-w-0 rounded-[var(--radius-control)] border border-[var(--color-border)] bg-[var(--color-surface-subtle)] p-4">
          ...
          {award.candidates.length > 0 ? <ul aria-label={t("awards.candidatesLabel", { award: t(`awards.names.${award.award}`) })} className="mt-4 grid gap-2" role="radiogroup">
            {award.candidates.map((candidate) => <li key={candidate.playerId} className="border-t border-[var(--color-border)] pt-3 first:border-t-0 first:pt-0">
              <label className="flex min-h-11 min-w-0 cursor-pointer items-start gap-3 rounded-[var(--radius-control)] miracle-focus-within-ring">
                <input checked={candidate.playerId === decision.playerId} className="mt-1 size-4 shrink-0 accent-[var(--color-brand-violet)]" name={`award-${award.award}`} onChange={() => onDecisionChange(award.award, { ...decision, playerId: candidate.playerId })} type="radio" value={candidate.playerId} />
          ...
          {award.tied ? <div className="mt-4 rounded-[var(--radius-control)] border border-[var(--color-brand-cream)]/40 p-3">
            ...
            <textarea className="mt-2 min-h-24 w-full resize-y rounded-[var(--radius-control)] border border-[var(--color-border-strong)] bg-[var(--color-surface)] p-3 text-sm text-[var(--color-text)] miracle-focus-ring" id={`awardReason-${award.award}`} maxLength={2000} name={`awardReason-${award.award}`} onChange={(event) => onDecisionChange(award.award, { ...decision, reason: event.target.value })} required value={decision.reason} />
