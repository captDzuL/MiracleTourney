      ...
      {view.mode === "drawing" ? (
        ...
        <>
          <section className={panel}>
            ...
            <p role="status" className="mt-3 text-sm text-white/60">{view.certificates.status === "published"
              ? (locale === "id" ? "Tujuh certificate resmi telah diterbitkan." : "All seven official certificates are published.")
              : (locale === "id" ? "Certificate sedang disiapkan organizer. Tautan akan tersedia setelah tujuh certificate diterbitkan bersama." : "Certificates are being prepared by the organizer. Links will appear after all seven are published together.")}</p>
            <ol className="mt-4 grid gap-3 md:grid-cols-3">{view.podium.map((placement) => <li key={placement.rank} className="border border-white/10 p-4"><span className="text-3xl font-black text-[#c7ff35]">#{placement.rank}</span><p className="mt-2 font-black">{placement.teamName}</p>{placement.certificate ? <div className="mt-4 flex flex-wrap gap-3"><a href={placement.certificate.publishedUrl} target="_blank" rel="noopener noreferrer" className="miracle-focus-ring inline-flex min-h-11 items-center text-sm font-bold text-cyan-300 underline">{locale === "id" ? "Lihat certificate tim" : "View team certificate"}</a><Link href={`/${locale}/certificates/verify/${placement.certificate.verificationCode}`} className="miracle-focus-ring inline-flex min-h-11 items-center text-sm font-bold text-white/70 underline">{locale === "id" ? "Verifikasi" : "Verify"}</Link></div> : null}</li>)}</ol>
          </section>
          <section className={panel}>
