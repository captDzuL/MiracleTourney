...
export function AdaptiveBracketBoard({ locale, format, matches, standings, registrationSlots = 0 }: { locale: "id" | "en"; format: string; matches: Match[]; standings: AdaptivePhaseStanding[]; registrationSlots?: number }) {
  ...
  return <div className="grid min-w-0 gap-6">
    ...
    {!league && rounds.length ? <section aria-labelledby="adaptive-bracket-heading" className="min-w-0"><h2 id="adaptive-bracket-heading" className="text-xl font-extrabold">Bracket</h2><div className="mt-4 w-full min-w-0 max-w-full overflow-x-auto overscroll-x-contain pb-2"><div className="flex min-w-max items-stretch gap-5">{rounds.map(([roundLabel, roundMatches]) => <section key={roundLabel} className="flex w-72 shrink-0 flex-col rounded-[var(--radius-control)] border border-[var(--color-border)] bg-[var(--color-surface-subtle)] p-4"><h3 className="font-extrabold">{roundLabel}</h3><div className="mt-4 flex flex-1 flex-col justify-around gap-4">{roundMatches.map((match) => <article key={match.id} className="rounded-[var(--radius-control)] border border-[var(--color-border)] bg-[var(--color-surface)] p-4"><p className="break-words font-bold">{match.home ?? "TBD"}</p><p className="my-2 text-sm text-[var(--color-text-muted)]">vs</p><p className="break-words font-bold">{match.away ?? "TBD"}</p>{match.status === "completed" ? <p className="mt-3 border-t border-[var(--color-border)] pt-3 text-xl font-extrabold tabular-nums">{match.homeScore} – {match.awayScore}</p> : <p className="mt-3 text-sm text-[var(--color-text-muted)]">{match.status === "live" ? "Live" : id ? "Menunggu hasil" : "Awaiting result"}</p>}</article>)}</div></section>)}</div></div></section> : null}
