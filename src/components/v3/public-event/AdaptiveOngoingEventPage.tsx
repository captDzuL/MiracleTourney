...
function MatchCard({ match, locale, timezone, t }: { match: PublicOngoingMatch; locale: "id" | "en"; timezone: string; t: Copy }) {
  return <article className="min-w-0 break-words rounded-[var(--radius-control)] border border-[var(--color-border)] bg-[var(--color-surface-subtle)] p-4">
    <div className="flex flex-wrap justify-between gap-2 text-sm"><span>{match.roundLabel} · BO{match.bestOf}</span><span className="font-bold text-[var(--color-brand-cyan)]">{t.statuses[match.status]}</span></div>
  ...
  return <div className="miracle-v3 adaptive-public-event grid min-w-0 gap-6 pb-12 text-[var(--color-text)]">
    ...
    <section id="ongoing-context" aria-labelledby="ongoing-context-heading" className={`${panel} scroll-mt-24`}>
      ...
      <div className="grid min-w-0 gap-5 lg:grid-cols-2">{state.standings.map(table => <section key={`${table.phaseId}:${table.groupId}`} className="min-w-0">
        <h3 className="text-lg font-bold">{table.label || t.brackets.round_robin}</h3>
      ...
      <div className="mt-5 grid min-w-0 gap-5 lg:grid-cols-2">{bracketGroups.map(([key, matches]) => <section key={key} className="min-w-0"><h3 className="mb-3 text-lg font-bold">{t.brackets[matches![0].bracket as keyof Copy["brackets"]]} · {matches![0].roundLabel}</h3><div className="grid gap-3">{matches!.map(match => <MatchCard key={match.id} match={match} locale={locale} timezone={timezone} t={t} />)}</div></section>)}</div>
