...
export function AdaptiveOngoingEventPage({ view, locale }: { view: PublicOngoingEventViewModel; locale: "id" | "en" }) {
  ...
  return <div className="miracle-v3 adaptive-public-event grid min-w-0 gap-6 pb-12 text-[var(--color-text)]">
    ...
    <nav aria-label={locale === "id" ? "Navigasi event" : "Event navigation"} className="flex flex-wrap gap-2">{[[base, t.overview], [`${base}/participants`, t.participants], [`${base}/schedule`, t.fullSchedule], [`${base}/${contextRoute}`, contextRoute === "bracket" ? t.bracket : t.standingsLink], [`${base}/leaderboards`, t.leaderboard]].map(([href, label]) => <a className={link} key={href} href={href}>{label}</a>)}</nav>
    ...
    {state.nextMatches.length > 12 ? <details className={panel}><summary className="miracle-focus-ring min-h-11 cursor-pointer py-2 font-bold">{locale === "id" ? "Sisa jadwal" : "Remaining schedule"} ({state.nextMatches.length - 12})</summary><div className="mt-4 grid min-w-0 gap-4 md:grid-cols-2 xl:grid-cols-3">{state.nextMatches.slice(12).map(match => <MatchCard key={match.id} match={match} locale={locale} timezone={timezone} t={t} />)}</div></details> : null}
