...
type ReviewFeedback = { message: string; error?: boolean };
export function PaymentReviewPanel({ locale, eventId, query, returnTo, payments }: { locale: "id" | "en"; eventId: string; query: RegistrationQuery; returnTo: string; payments: PaymentReviewEntry[] }) {
 ...
 const [selectedId, setSelectedId] = useState(rows[0]?.id); const selected = rows.find(row => row.id === selectedId) ?? rows[0]; const [feedback, setFeedback] = useState<ReviewFeedback>({ message: "" });
...
function PaymentDetail({ locale, eventId, returnTo, entry, onFeedback, refresh }: { locale: string; eventId: string; returnTo: string; entry: PaymentReviewEntry; onFeedback: (feedback: ReviewFeedback) => void; refresh: () => void }) {
 const t = useTranslations("registrationWorkspace"); const [status, setStatus] = useState(entry.status); const [reason, setReason] = useState(""); const [busy, setBusy] = useState(false); const [zoom, setZoom] = useState(false); const reasonRef = useRef<HTMLTextAreaElement>(null);
 ...
 async function review(reject: boolean) {
  if (locked) return;
  if (reject && reason.trim().length < 3) { onFeedback({ message: t("reasonRequired"), error: true }); reasonRef.current?.focus(); return; }
  setBusy(true); onFeedback({ message: "" });
  ...
  try { const result = await (reject ? rejectEventPaymentAction(data) : approveEventPaymentAction(data));
    if (result.status === "approved" || result.status === "rejected") { setStatus(result.status); onFeedback({ message: t("decisionSaved") }); }
    else { onFeedback({ message: t(result.status === "conflict" ? "conflict" : "operationFailed"), error: true }); if (result.status === "conflict") { setStale(true); refresh(); } }
