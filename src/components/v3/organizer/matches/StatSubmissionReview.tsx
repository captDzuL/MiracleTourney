...
export function StatSubmissionReview({submission,data,locale,disabled,onReview}:{submission:EventMatchStatistics["submissions"][number];data:EventMatchStatistics;locale:"id"|"en";disabled:boolean;onReview:(form:FormData,decision:"approve"|"reject")=>Promise<void>}) {
 ...
 return <form data-submission={submission.id} className={surface+" grid gap-4"} onSubmit={event=>{event.preventDefault();review(event.currentTarget,"reject");}}>
  ...
  <p className="text-sm text-[var(--color-text-subtle)]">{t("submitted",{time:new Intl.DateTimeFormat(locale,{dateStyle:"medium",timeStyle:"short"}).format(new Date(submission.submittedAt))})}</p>
  ...
  </>:<>{submission.rejectionNote&&<p className="break-words">{submission.rejectionNote}</p>}{submission.reviewedAt&&<p className="text-sm">{t("reviewed",{time:new Intl.DateTimeFormat(locale,{dateStyle:"medium",timeStyle:"short"}).format(new Date(submission.reviewedAt))})}</p>}</>}
 </form>;
}
