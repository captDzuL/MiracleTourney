...
import { mutateCompetitionWorkspaceAction, type CompetitionMutationActionResult } from "@/lib/actions/competition-v3-actions";
...
type Props = { eventId: string; version: number; announcements: readonly Announcement[]; audit: CompetitionWorkspaceState["audit"]; action?: (input: unknown) => Promise<CompetitionMutationActionResult> };
type Urgency = "info" | "important" | "urgent";
type MutationOutcome = CompetitionMutationActionResult | undefined;
...
export function AnnouncementsWorkspace({ eventId, version, announcements, audit, action = mutateCompetitionWorkspaceAction }: Props) {
  ...
  const run = (command: Record<string, unknown>): Promise<MutationOutcome> => {
...
function AnnouncementItem({ announcement, pending, t, onCommand }: { announcement: Announcement; pending: boolean; t: (key: string) => string; onCommand: (command: Record<string, unknown>) => Promise<MutationOutcome> }) {
