...
const FOLDER = "event-backgrounds";
...
export type UploadedVisualImageAsset = {
  url: string;
  mimeType: string;
  width: number;
  height: number;
...
/**
 * Single validation + storage boundary for event background uploads. Checks,
 * in order: entity id shape, presence, byte size, declared MIME, magic bytes,
 * and finally real decodability through `sharp` (which also gives the
 * dimensions persisted on the visual revision).
 */
export async function uploadEventVisualImage({
  file,
  entityId,
  maxBytes,
  errorPath,
  ...
  maxBytes: number;
  errorPath: string;
}): Promise<UploadedVisualImageAsset> {
