...
async function uploadImageAsset({
  ...
  if (!(file instanceof File) || file.size === 0) {
    ...
    throw new Error(`No ${label} file uploaded.`);
  ...
  if (!hasImageSignature(buffer, extension)) {
    ...
    throw new Error(`${label} file content does not match its image type.`);
...
export async function captainRegisterTeamAction(formData: FormData) {
  ...
  const input = {
    ...
    tag: parsed.data.tag?.trim().toUpperCase() ?? "",
...
export async function captainSaveDraftTeamAction(formData: FormData) {
  ...
  try {
    ...
    await createOrUpdateCaptainDraftTeam({ captainId: captain.id, captainName: captain.name, name: input.name, tag: input.tag });
...
export async function captainUploadTeamLogoAction(formData: FormData) {
  ...
  try {
    ...
    await updateCaptainTeamLogo(captain.id, teamId, url);
...
export async function adminAssignCaptainAction(formData: FormData) {
  ...
  if (!team?.eventId) {
    ...
    return redirectToActiveLocale(`/admin?error=${encodeURIComponent("Tim event tidak ditemukan.")}` as never);
...
export async function adminDeleteTeamAction(formData: FormData) {
  ...
  if (!team?.eventId || !team.event) {

    return redirectToActiveLocale(`/admin?error=${encodeURIComponent("Draft tim captain tidak dapat dihapus dari panel event.")}` as never);
