...
describe("organizer event ownership", () => {
  ...
  it("updates a team logo only when the team belongs to the captain", async () => {
    ...
    await expect(updateCaptainTeamLogo("captain-1", "team-1", "/team-logos/team-1.png")).resolves.toMatchObject({
  ...
  it("rejects a captain team logo update when the team belongs to another captain", async () => {

    prisma.team.findFirst.mockResolvedValue(null);



    await expect(updateCaptainTeamLogo("captain-1", "team-other", "/team-logos/team-other.png")).rejects.toThrow("Not authorized");
...
describe("existing captain event registration", () => {
  ...
  it("copies a captain draft team and UID/IGN roster when registering for an event", async () => {
    ...
    prisma.team.findFirst
      ...
      .mockResolvedValueOnce({
        ...
        players: [
          ...
          { id: "p1", teamId: "draft-1", eventId: null, displayName: "123456", nickname: "Ace", position: "", jerseyNumber: null },
    ...
    await expect(registerTeam({ eventId: "event-open", captainId: "captain-1", draftTeamId: "draft-1" })).resolves.toEqual(

      expect.objectContaining({ id: "team-new", eventId: "event-open", tag: "DU" }),

    );

    expect(prisma.player.createMany).toHaveBeenCalledWith({

      data: [expect.objectContaining({ teamId: "team-new", eventId: "event-open", displayName: "123456", nickname: "Ace", position: "" })],
  ...
  it("creates or updates a reusable captain draft team without an event", async () => {
    ...
    const result = await createOrUpdateCaptainDraftTeam({ captainId: "captain-1", captainName: "Captain", name: "Draft United", tag: "du" });

    expect(result).toEqual(expect.objectContaining({ id: "draft-1", source: "draft", tag: "DU" }));

    expect(result.eventId).toBeUndefined();

    expect(prisma.team.create).toHaveBeenCalledWith({

      data: expect.objectContaining({ eventId: null, captainId: "captain-1", source: "draft", tag: "DU" }),
