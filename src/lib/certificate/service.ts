...
export function validateCertificateAssetPlacement(
  ...
  if (!asset.storageOwnershipVerified || !isSafeOwnedAssetUrl(asset.url)) return { success: false, code: "unsafe_asset" };
...
}).strict().refine((input) => Boolean(input.assetId) === Boolean(input.placement), { message: "Asset and placement must be supplied together" });
...
export interface CertificateStudioCompletion { readonly status: "completed" | "reopened" | "editable"; readonly version: number; readonly certificateRevision: number }
...
export type PublishCertificateSetResult =
  ...
  | { readonly status: "integration_required" };
export interface CertificateStudioMutation { readonly fingerprint: string; readonly actorId: string; readonly result: RegenerateCertificateResult | PublishCertificateSetResult }
...
export interface CertificateStudioTransaction {
  ...
  appendVersion(input: { certificateType: MiracleV3CertificateType; idempotencyKey: string; fingerprint: string; actorId: string; placement: CertificateAssetPlacement | null; asset: TrustedCertificateAsset | null }): Promise<{ record: CertificateStudioRecord; data: MiracleV3CertificateData }>;
...
export async function regenerateCertificate(input: unknown, dependencies: CertificateStudioDependencies): Promise<RegenerateCertificateResult> {
  ...
  const fingerprint = mutationFingerprint({ action: "regenerate", certificateType: value.certificateType, expectedVersion: value.expectedVersion, placement: value.placement ?? null, assetId: value.assetId ?? null });
  const prepared = await dependencies.transaction(value.eventId, async (tx) => {
    ...
    if (existing) {
      ...
      return { terminal: { status: "already_applied", result: existing.result as RegenerateCertificateResult } as RegenerateCertificateResult };
    }
    ...
    let placement: CertificateAssetPlacement | null = null;
    let trustedAsset: TrustedCertificateAsset | null = null;
    if (value.assetId && value.placement) {
      const teamCertificate = ["champion", "runner_up", "third_place"].includes(value.certificateType);
      if ((teamCertificate && value.placement.assetKind !== "team_logo_hero")
        || (!teamCertificate && value.placement.assetKind === "team_logo_hero")) return { terminal: { status: "blocked", code: "invalid_asset" } as RegenerateCertificateResult };
      const asset = await tx.resolveAsset?.(value.assetId);
      if (!asset || !validateCertificateAssetPlacement(value.placement, asset).success) return { terminal: { status: "blocked", code: "invalid_asset" } as RegenerateCertificateResult };
      placement = value.placement;
      trustedAsset = asset;
    }
    return tx.appendVersion({ certificateType: value.certificateType, idempotencyKey: value.idempotencyKey, fingerprint, actorId: actor.id, placement, asset: trustedAsset });
  });
...
export async function publishCertificateSet(input: unknown, dependencies: CertificateStudioDependencies): Promise<PublishCertificateSetResult> {
  ...
  return dependencies.transaction(value.eventId, async (tx) => {
    ...
    const ready = records.length === 7 && selection.every((selected) => {
      ...
      return record?.eventId === value.eventId && (record.status === "ready" || record.status === "published")
