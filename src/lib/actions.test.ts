...
describe("captainSignUpAction", () => {
  ...
  it("creates a captain account and redirects to /captain?success=registered on valid input", async () => {
  ...
  it("does not require event or team fields during account registration", async () => {

    await expect(captainSignUpAction(fd(validData))).rejects.toThrow("REDIRECT:/captain?success=registered");

    expect(createCaptainAccount).toHaveBeenCalledTimes(1);
...
describe("captain actions", () => {
  ...
  it("can register an event using an existing draft team", async () => {

    await expect(

      captainRegisterTeamAction(fd({ eventId: "event-flashpeak-open", draftTeamId: "draft-1" })),
  ...
  it("saves a captain draft team outside any event", async () => {

    await expect(captainSaveDraftTeamAction(fd({ name: "Draft United", tag: "DU" }))).rejects.toThrow(

      "REDIRECT:/captain?tab=roster&success=draft-team-saved",

    );

    expect(createOrUpdateCaptainDraftTeam).toHaveBeenCalledWith({
  ...
  it("allows a captain to upload a logo for their own team", async () => {

    const image = new File([new Uint8Array([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a])], "logo.png", { type: "image/png" });



    await expect(captainUploadTeamLogoAction(fd({ teamId: "team-1", teamLogo: image }))).rejects.toThrow(

      "REDIRECT:/captain?tab=roster&success=team-logo-updated",

    );



    expect(updateCaptainTeamLogo).toHaveBeenCalledWith(

      "captain-1",

      "team-1",

      expect.stringMatching(/^\/team-logos\/team-1-\d+\.png$/),
  ...
  it("requires a captain session before uploading a team logo", async () => {
    ...
    await expect(captainUploadTeamLogoAction(fd({ teamId: "team-1", teamLogo: image }))).rejects.toThrow("REDIRECT:/login");
  ...
  it("rejects spoofed captain logo uploads before updating the team", async () => {

    const image = new File(["not an image"], "logo.png", { type: "image/png" });



    await expect(captainUploadTeamLogoAction(fd({ teamId: "team-1", teamLogo: image }))).rejects.toThrow(

      "REDIRECT:/captain?tab=roster&error=",
  ...
  it("requires a captain session before adding a player", async () => {
    ...
    await expect(
      ...
      captainAddPlayerAction(fd({ teamId: "t1", eventId: "e1", displayName: "123456", nickname: "PL", position: "" })),
  ...
  it("adds player and redirects on success", async () => {
    ...
    await expect(
      ...
      captainAddPlayerAction(
        ...
        fd({ teamId: "t1", eventId: "e1", displayName: "123456789", nickname: "Dhani", position: "" }),
    ...
    expect(addPlayer).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ displayName: "123456789", nickname: "Dhani", position: "" }),
...
describe("captainUpdatePlayerAction", () => {
  ...
  it("updates player and redirects to /captain?success=player-updated", async () => {
    ...
    expect(updatePlayer).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ displayName: "987654321", position: "" }),
