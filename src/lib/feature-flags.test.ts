...
describe("public visual feature flags", () => {
  ...
  it("keeps public discovery V3 disabled until explicitly enabled", () => {
    expect(isFeatureEnabled("public_discovery_v3")).toBe(false);

    process.env.FEATURE_FLAG_PUBLIC_DISCOVERY_V3 = "true";
    expect(isFeatureEnabled("public_discovery_v3")).toBe(true);
  });
