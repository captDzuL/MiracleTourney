...
| `src/lib/registration/actions.ts` | captain session | current captain and event registration | bounded event/team/contact fields and validated import payloads | same-origin server action | registration/import limiter | stable registration result codes | mutation invalidates registration/team tags |
