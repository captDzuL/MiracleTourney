...
Status: bounded coherent slice complete through Task 8 fix round 3; full runtime/browser/live-database gate remains BLOCKED.
...
- Inventory: 6 API route rows + 15 listed action/validation/import boundaries = 21 base rows in `docs/operations/release-security-inventory.md`, with explicit round-2 and round-3 coverage tables below. This is a bounded inventory, not an exhaustive action sweep.
- Implemented production slice: reusable same-origin guard with fail-closed unsafe-request policy, public error mapper, structured redacted logger, minimal health response, uniform route error envelopes, competition path validation, credential-export safe errors/formula neutralization, ongoing-event slug/error validation, safe action/registration errors, actor-scoped upload/import/certificate throttles before downstream work, strict nested player-stats/tournament IDs, private auth/provider failure responses, plain-text team-field validation, and CSV/XLSX/image filename/formula/signature validation.
...
| Finding | Covered code/tests | Result |
| --- | --- | --- |
| P1 upload/import limiter placement | `src/lib/actions.ts`, `src/lib/actions/certificate-v3-actions.ts`; blocked-path tests for payment proof, captain/admin/team/event logo, event visual, character art, CSV import, and certificate asset | GREEN; exact named boundaries covered, zero blob/parser/repository calls on blocked paths |
| P1 strict nested entity IDs | `src/lib/actions/player-stats-v3-actions.ts`, `src/lib/tournament/operations/schema.ts`, `src/lib/tournament/operations/index.ts`; SQL/traversal tests for match/team/submission and match/team/revision/incident/action/announcement | GREEN; player 17/17 and competition 14/14 action suites |
| P2 auth/provider failure and private headers | captain-credentials, health/env, debug-locale routes and tests | GREEN; 15 route tests including auth/locale-provider rejection and success/forbidden `no-store` + `Vary: Cookie` assertions |
...
| Round-3 limiter placement | payment-proof regression: 1 failure/1 before the check; current action test expected rate-limited but received upload fallback | GREEN; blocked-path upload/import suite covers 14 selected rate-limit cases |
| Round-3 nested IDs | six competition action cases reached one `$transaction` each before schema tightening | GREEN; 6/6 reject before transaction, plus player-stats 3/3 nested cases |
| Round-3 route dependency/cache | 9 failures across 14 route tests: uncaught auth rejection and missing `Vary: Cookie`/success cache headers | GREEN; 15/15 route tests |
...
| round-3 focused action/route suite | exit 0 | 7 files, 226 tests passed, 0 skipped, 0 failed |
| round-3 TypeScript | exit 0 | `node node_modules/typescript/bin/tsc --noEmit`; no diagnostics |
| round-3 focused ESLint | exit 0 | all changed round-3 source/tests; no errors or warnings |
| round-3 `git diff --check` | exit 0 | no whitespace errors; Git emitted only CRLF normalization warnings |
| round-3 raw scan | exit 1 with no matches | no runtime `$queryRawUnsafe`, `$executeRawUnsafe`, or wildcard authenticated CORS header |
...
- Health now returns `{ status: "ok" }` only to a platform-admin session and `no-store, max-age=0` + `Vary: Cookie` headers; auth failures are generic.
...
- Certificate V3 regeneration/publication/upload and every exact round-3 registration/import/upload boundary use the shared in-process limiter before downstream work; browser/load behavior still needs authorized runtime evidence.
- Public player-stats and tournament operation boundaries reject unsafe nested IDs before repository/transaction calls while preserving bounded server-issued graph match IDs and separate opaque/descriptive values.
- Captain-credentials and debug-locale success/forbidden/provider-failure responses are private and non-cacheable with `Vary: Cookie`.
...
- The exact round-3 limiter list is covered; other legacy action boundaries remain outside this direct-call matrix and are not claimed exhaustive.
...
- Round-3 additions: exact upload/import limiter guards and blocked-path tests in `src/lib/actions.ts`/certificate actions, strict nested player-stats/tournament operation schemas/tests, and auth/provider/cache contract tests for captain credentials, health/env, and debug locale.

Initial Task 8 commit: `796668207d9607fa39c198ea0dc201f84fe6987e` (`fix: harden API inputs and public errors`). Round-1 fix commit: `976141ec530acbe8a1ba3b443fe6ab632abe3a7b` (`fix: close task 8 security review findings`). Round-2 baseline: `66b2412b189644a6c523110336c30b9aed01067b` (`fix: close task 8 round 2 security findings`). Round-3 commit contains the final bounded changes and this evidence report.
