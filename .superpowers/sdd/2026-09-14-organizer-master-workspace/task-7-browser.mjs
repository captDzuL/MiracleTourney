...
const out=path.join(repo,".superpowers/sdd/2026-09-14-organizer-master-workspace/task-7-browser");
...
const locale=location.pathname.startsWith("/id/")?"id":"en",query=Object.fromEntries(new URLSearchParams(location.search)),view=location.pathname.split("/").at(-1),preset=query.preset||"groupPlayoffs";
...
try{
for(const locale of ["id","en"])for(const width of [360,390,768,1024,1440])for(const view of ["competition","schedule","match-control"])for(const preset of ["singleElimination","doubleElimination","roundRobin","groupPlayoffs"]){
 ...
 if(view==="match-control"){
   const item=page.locator("[data-match-row]").nth(1);await item.focus();await item.press("Enter");
   await page.waitForURL(/match=/);await page.locator("[data-match-detail]").waitFor();
   const selected=new URL(page.url()).searchParams.get("match");
   if(!decodeURIComponent(await page.locator("[data-match-detail]").getAttribute("href")).includes("/matches/"+selected+"?view=result"))throw Error("Stable selection link");
   if(!await page.locator('[data-match-row][aria-current="true"]').count())throw Error("Selection highlight");
   await page.goBack();await page.locator("[data-operations]").waitFor();
   await page.locator('select[name="filter"]').selectOption("needs-result");
   await page.locator('[data-operations] form button[type="submit"]').click();await page.waitForURL(/filter=needs-result/);
   await page.locator("[data-operations]").waitFor();
   if(await page.locator("[data-match-row]").count()!==1)throw Error("URL filter");
 }
 if([360,1440].includes(width)&&preset==="groupPlayoffs")await page.screenshot({path:path.join(out,`${locale}-${width}-${view}.png`),fullPage:true});
 results.push({locale,width,view,preset,...geometry});await page.close();
}
if(errors.length)throw Error(errors.join("\n"));
await fs.writeFile(path.join(out,"results.json"),JSON.stringify(results,null,2));console.log(JSON.stringify({cases:results.length,overflow:0,smallTargets:0,keyboard:"passed",selectionAndFilters:"passed",errors}));
